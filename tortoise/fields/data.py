...
from typing import TYPE_CHECKING, Any, Callable, Optional, Type, TypeVar, Union, List
...
class ArrayField(Field):  # type: ignore
    ...
    def __init__(self, elem_type: Type, **kwargs: Any) -> None:
        if not issubclass(elem_type, (int, str)):
            raise ConfigurationError(
                "ArrayField only supports integer or string or Enums of these!"
            )
        super().__init__(**kwargs)
        self.elem_type = elem_type

    def to_db_value(
        self, value: List[Union[int, str, Enum]], instance
    ) -> List[Union[int, str]]:
    ...
    def to_python_value(
        self, value: List[Union[int, str]]
    ) -> List[Union[int, str, Enum]]:
        if value and issubclass(self.elem_type, Enum):
